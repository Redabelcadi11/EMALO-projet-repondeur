...
def main() -> int:
    ...
    selected = unseen_rows[:100]
    if len(selected) < 100:
        raise RuntimeError(f"Impossible de trouver 100 audios inedits: {len(selected)}")
        ...
        eval_rows.append(
            {
                ...
                "source": "nextcloud_unseen100_no_leak",
    ...
    if len(eval_rows) < 100:
        raise RuntimeError(f"Appariement incomplet: {len(eval_rows)}/100")
    ...
    for index, eval_row in enumerate(eval_rows, start=1):
        ...
        print(f"DONE {index}/100 in {elapsed:.2f}s :: {eval_row['audio']}", flush=True)
    ...
    report = {
        ...
        "selection_policy": "100 derniers audios inedits hors corpus courant",
    ...
    output = state_dir / "UNSEEN100_NOLEAK_EVAL.json"
