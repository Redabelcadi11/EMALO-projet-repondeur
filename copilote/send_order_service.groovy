...
if (!guardFile) {
    throw new SecurityException("[ERP_WRITE_BLOCKED] creation de commande par service: garde central introuvable")
}
def erpGuard = new GroovyShell().parse(guardFile)
erpGuard.assertErpWriteAllowed(guardFile.parentFile.parentFile, 'creation de commande par service Groovy')
