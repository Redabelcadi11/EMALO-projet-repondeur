...
class RepondeurApp(tk.Tk):
    ...
    def send_refs(self, refs: list[str]) -> None:
        if not refs:
            ...
            return
        try:
            assert_erp_write_allowed("interface Python: envoi de commandes Copilote")
        except ERPWriteBlocked as exc:
            messagebox.showerror("Ecriture ERP bloquee", str(exc))
            self.log(str(exc))
            return
    ...
    def run_nextcloud_sync(self) -> None:
        env_extra = {"NEXTCLOUD_USERNAME": os.environ.get("NEXTCLOUD_USERNAME", "bascogel")}
