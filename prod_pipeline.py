...
def summarize_command(commande: dict[str, Any], audio: Path) -> dict[str, Any]:
    ...
    lignes_par_ordre = {
        int(line.get("ordre_ligne") or 0): line
        for line in commande.get("lignes_commande", [])
        if str(line.get("ordre_ligne") or "").isdigit()
    }
    products = [
        {
        ...
        product_recognition.append(
            {
                ...
                "product_code": ligne.get("code_article", "") if recognized else "",
                "product_label": ligne.get("libelle_article", "") if recognized else "",
                "quantity": ligne.get("quantite", "") if recognized else produit.get("quantite_resolue", produit.get("quantite_principale", "")),
                "unit": ligne.get("unite", "") if recognized else produit.get("unite_resolue", produit.get("unite_principale", "")),
