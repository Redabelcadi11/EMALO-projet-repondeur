...
def _phrase_distinctive_produit(valeur: Any) -> str:
    """Extrait une courte designation commandable d'un libelle article.

    Les mots isoles aident Whisper, mais une longue commande contient aussi
    des noms composes (``caille brebis mamia``, ``emmental rape``,
    ``mangue cube``).  Cette fonction ne cree aucune variante ni aucune
    reference : elle conserve seulement les premiers mots distinctifs du
    libelle du cadencier deja autorise pour le client.
    """
    distinctifs = list(_tokens_distinctifs(valeur).values())
    # Au-dela de cinq mots, un hotword devient plutot une phrase artificielle
    # qu'une aide de decodage. Les marques et le conditionnement restent
    # accessibles par les hotwords unitaires deja produits ci-dessous.
    return " ".join(distinctifs[:5]).strip()
...
def construire_hotwords_par_telephone(
    ...
    termes_metier_globaux = [
        # Les synonymes globaux completent le cadencier ; ils ne doivent pas
        # evincer les produits specifiques du client lorsque la liste est
        # limitee.
        (10.0, str(canonique).strip())
    ...
    for client in clients:
        ...
        # Les deux sources sont connues *avant* la transcription :
        # - ``telephones`` provient d'info-clients / du fichier de telephones ;
        # - ``telephones_confirmes`` provient des associations explicitement
        #   validees.  Le second doit alimenter Whisper au meme titre que le
        #   premier : attendre le texte ASR pour l'utiliser annulerait son
        #   principal interet (vocabulaire du cadencier du bon client).
        telephones = {
        ...
        meilleures_phrases: dict[str, tuple[float, str]] = {}
        for produit in cadencier.get(code_client, []):
            ...
            phrase = _phrase_distinctive_produit(
                produit.get("libelle_article")
            )
            if phrase:
                cle_phrase = _normaliser_token(phrase)
                # Les phrases client passent avant les mots unitaires et les
                # synonymes globaux. Leur nombre est borne plus bas : elles
                # ne doivent jamais evincer tout le vocabulaire atomique.
                score_phrase = (
                    300.0
                    + math.log1p(recent)
                    + 0.25 * math.log1p(total)
                )
                precedent_phrase = meilleures_phrases.get(cle_phrase)
                if (
                    precedent_phrase is None
                    or score_phrase > precedent_phrase[0]
                ):
                    meilleures_phrases[cle_phrase] = (
                        score_phrase,
                        phrase,
                    )
        ...
        # Reserve explicitement une place aux expressions composees du
        # cadencier.  Ainsi, une liste longue peut etre decoupee/reprise par
        # Whisper sans perdre les termes qui font le produit (MANGUE CUBE,
        # EMMENTAL RAPE, BOBINE ESSUIE MAINS...).
        limite_phrases = min(
            len(meilleures_phrases),
            max(12, min(48, max(1, limite_termes // 3))),
        )
        termes_client.extend(
            sorted(
                meilleures_phrases.values(),
