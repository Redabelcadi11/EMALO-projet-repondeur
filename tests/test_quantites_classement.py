...
def test_decimal_du_libelle_brut_reste_deux_litres_et_demi() -> None:
    meta = _analyser_conditionnement_article(
        {
            "libelle_article": "2.5L YAOURT CREME GLACEE",
            "libelle_normalise": "2 5l yaourt creme glacee",
            "unite_vente": "BOITE",
        }
    )
    assert meta["taille_l_par_unite"] == 2.5


def test_quinze_litres_en_bacs_de_deux_litres_et_demi_font_six_bacs() -> None:
    resultat = _resoudre_quantite_commande_candidat(
        {
            "quantite_principale": 15,
            "unite_principale": "L",
            "conditionnement_multiple": None,
            "texte_source": "15 litres de glace yaourt",
        },
        {
            "code_article": "00020272",
            "libelle_article": "2.5L YAOURT BULGARE CREME GLACEE",
            "libelle_normalise": "2 5l yaourt bulgare creme glacee",
            "unite_vente": "BOITE",
        },
    )
...
def test_conditionnement_inverse_deux_kilos_fois_cinq_poches() -> None:
    resultat = _resoudre_quantite_commande_candidat(
        {
            "quantite_principale": 20,
            "unite_principale": "KG",
            "conditionnement_multiple": None,
            "texte_source": "20 kilos de glacons",
        },
        {
            "code_article": "00005016",
            "libelle_article": "GLACON 2K X5P",
            "libelle_normalise": "glacon 2k x5p",
            "unite_vente": "COL",
        },
    )
    assert (resultat["quantite_resolue"], resultat["unite_resolue"]) == (
