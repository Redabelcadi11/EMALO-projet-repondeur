...
def test_synonyme_cadencier_departage_par_ventes_client() -> None:
    ...
    assert resultats[0]["selection"]["code_article"] == "P1"
    assert (
...
def test_quantite_absente_prend_habitude_client() -> None:
    mentions = [
        {
    ...
    assert resultats[0]["selection"] is not None
    assert resultats[0]["quantite_resolue"] == 2.0
    assert resultats[0]["unite_resolue"] == "PCE"
