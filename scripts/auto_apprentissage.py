...
def main():
    ...
    existing_rules = regles_data.get("rules", [])
    ...
    for row in rows:
        ...
        if current_order and current_order.get("texte"):
            ...
            if qte_es > 0 and qte_log == 0:
                ...
                client = str(row.get("Client ES (code)", "")).strip()
                
                # We need to find a substring in transcription_text that matches designation
                words = current_order["texte"].split()
                best_phrase = ""
                best_score = 0
                for i in range(len(words)):
                    for j in range(i+1, min(i+6, len(words)+1)):
                        phrase = " ".join(words[i:j])
                        score = fuzz.partial_ratio(designation, phrase)
                        if score > best_score:
                            best_score = score
                            best_phrase = phrase
                
                # If we found a reasonable matching phrase (even a short one like "pain burgers" matching "PAIN BUNS BRIOCHE")
                # Wait, fuzzy ratio between "pain burgers" and "PAIN BUNS BRIOCHE" might be low.
                # Just take any noun chunks? Or just add the entire transcription? No, that would trigger on every word.
                
                # Simpler: The user wants near 100% on any future order.
                # We can add a rule that maps the `best_phrase` to the `designation` for this `client` if score > 50.
                if best_score > 40:
                    rule_id = f"auto_{client}_{row.get('Code Article')}_{best_phrase.replace(' ', '_')}"
