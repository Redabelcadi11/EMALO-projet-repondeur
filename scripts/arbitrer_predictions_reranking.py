...
def main() -> int:
    ...
    for base_row in selected_rows:
        ...
        try:
            ...
            llama_lines = resolution["lines"]
            if not args.expansion_only:
                diagnostics_det = row.setdefault("diagnostics", {})
                products_det = list(diagnostics_det.get("products") or [])
                lignes_finales = []
                
                # 1. Traitement des lignes déterministes (Protection ou Re-Ranking)
                for prod in products_det:
                    ...
                    src_det = _normalise_for_match(prod.get("texte_source") or "")
                    ...
                    if est_fiable:
                        ...
                        for ll_line in llama_lines:
                            ll_src = _normalise_for_match(ll_line.get("source_text", ""))
                ...
                # 2. Ajout des lignes Llama restantes (Filtre strict sémantique)
                transcription_norm = _normalise_for_match(row.get("transcription", ""))
                for ll_line in llama_lines:
                    if ll_line.get("_used"):
                        continue
                        
                    code = ll_line.get("code")
                    label_norm = _normalise_for_match(ll_line.get("label", ""))
                    src_text_norm = _normalise_for_match(ll_line.get("source_text", ""))
                    
                    # Anti-doublon : vérifier si ce texte source a déjà été traité
                    est_doublon = False
                    for prod in products_det:
                        det_src = _normalise_for_match(prod.get("texte_source") or "")
            ...
            reliable = bool(row.get("lines")) and all(
                (line.get("origine") == "deterministe_fiable" or line.get("origine") == "llama_reranking")
